<template>
  <el-form ref="templateForm" :model="templateForm" :rules="rules" label-width="88px" class="bargain-form">
      ...
      <div class="el-form-item__content" style="margin-left: 88px;">
        ...
        <div v-for="(item, index) in templateForm.prizeInfo.courses" :key="item.id" class="course-item">
          ...
          <el-form-item label="购买课时">
            <el-input-number v-model="item.classHour" :controls="false" :min="0" :max="9999" step-strictly class="left-number" style="width: 203px;"></el-input-number>
          </el-form-item>
          <el-form-item label="总价格(元)">
            <el-input-number v-model="item.price" :controls="false" :min="0" :max="9999" :precision="2" @change="calcSinglePrice(item)" class="left-number" style="width: 203px;"></el-input-number>
          </el-form-item>
        ...
        <div v-for="(item, index) in templateForm.prizeInfo.tuitions" :key="item.id" class="course-item">
          ...
          <el-form-item label="购买数量">
            <el-input-number v-model="item.tuitionNum" :min="0" :max="9999" :controls="false" step-strictly @change="calcTuitionPrice(item)" class="left-number" style="width: 203px;"></el-input-number>
          </el-form-item>
          <el-form-item label="总价格(元)">
            <el-input-number v-model="item.price" :controls="false" :min="0" :max="9999" :precision="2" class="left-number" style="width: 203px;"></el-input-number>
          </el-form-item>
          <el-button type="danger" icon="el-icon-delete" plain circle @click="removeTuition(index)" class="delete-btn" title="删除课程"></el-button>
        </div>
        <p class="form-tip">需要选择课程或者学杂后，才支持线上支付。（如果没有选择课程、学杂，就只能下线支付）</p>
      </div>
    </div>
    <!-- 课程详情 -->
    ...
    <el-form-item label="活动原价" prop="prizeInfo.originalPrice">
      <el-input-number v-if="activityInfo && activityInfo.status != 0" :value="templateForm.prizeInfo.originalPrice" disabled :controls="false" :precision="2" :min="0" :max="99999999" class="left-number" style="width: 203px;"></el-input-number>
      <el-input-number v-else-if="isSelectedCourse" :value="totalPrice" disabled :controls="false" :precision="2" :min="0" :max="99999999" class="left-number" style="width: 203px;"></el-input-number>
      <el-input-number v-else v-model="templateForm.prizeInfo.originalPrice"  :controls="false" :precision="2" :min="0" :max="99999999" class="left-number" style="width: 203px;"></el-input-number>
      <p class="form-tip">活动发布后，原价不可更改</p>
    </el-form-item>
    <el-form-item label="活动底价" prop="prizeInfo.floorPrice">
      <el-input-number v-model="templateForm.prizeInfo.floorPrice" :min="0" :max="99999999" :disabled="activityInfo && activityInfo.status != 0" :controls="false" :precision="2" class="left-number" style="width: 203px;"></el-input-number>
      <p class="form-tip">活动发布后，底价不可更改</p>
    </el-form-item>
    <el-form-item label="活动名额" prop="prizeInfo.num">
      <el-input-number v-model="templateForm.prizeInfo.num" :controls="false" step-strictly :min="activityInfo && activityInfo.status != 0 ? templateForm.prizeInfo.num : 1" :max="9999" class="left-number" style="width: 203px;"></el-input-number>
      <p class="form-tip">活动发布后，活动名额只可增加不能减少</p>
    </el-form-item>
    <el-form-item label="砍价次数" prop="prizeInfo.totalNumber">
      <el-input-number v-model="templateForm.prizeInfo.totalNumber" :disabled="activityInfo && activityInfo.status != 0" :controls="false" :min="1" :max="999" step-strictly class="left-number" style="width: 203px;"></el-input-number>
      <p class="form-tip">从原价到底价一共要砍价的次数</p>
    </el-form-item>
    <!-- 活动规则 -板块 -->
    <div class='form-title'>
    ...
    <!-- 机构信息-板块 -->
    <div class='form-title'>
      <span class="color-primary">机构信息</span>
    </div>
    <el-form-item
...
export default {
    ...
    return {
      ...
      templateForm: {
        ...
        basicInfo: {
          ...
          endTime: '',
          musicType: 1,
          musicSrc: 'http://cdn.yizhiniao.com/activity/dogmusic.mp3',
          telephone: ''
        },
        prizeInfo: {
          courses: [],
        ...
        organInfo: [ // 机构信息
          { type: 0, value: 'XX教育培训创办于2222年，专注于少年儿童的兴趣教育，致力于让孩子更快乐、更轻松的学习，采用师生优化配对，根据学员年龄、性格特点、等实际情况推荐合适的老师，制定适合学员的教学方案及辅导考级。' },
          { type: 1, value: 'http://tmpcdn.tlybond.com/template/leaflet/summer_class/img/organ_lis.png' }
        ],
        reservationInfo: [],
        shareInfo: {
          imageUrl: 'http://cdntest.yizhiniao.com/oa_120_1536824614333_picture',
          title: 'XX教育培训秋招大促',
      ...
      rules: {
        ...
        'prizeInfo.floorPrice': { required: true, message: '请填写活动底价', trigger: 'blur' },
        'prizeInfo.totalNumber': { required: true, message: '请填写砍价次数', trigger: 'blur' }
      },
      playing: false,
      timeTypeList: [
        {
  ...
  mounted() {
    if (this.templateData != null) {
      ...
      temp.activityTimeRange = [temp.basicInfo.startTime || moment().format('YYYY-MM-DD'), temp.basicInfo.endTime || moment().add(7, 'days').format('YYYY-MM-DD')]
  ...
  methods: {
    publish() {
      // 发布活动
      this.$http.put('/v1/OrganizationReduceWeb/putawayOrSoudOut?status=1&id=' + this.activityId).then(() => {
        ...
        this.$refs.templateForm.validate(valid => {
          if (valid) {
            ...
            if (tempTemplateForm.prizeInfo.originalPrice <= tempTemplateForm.prizeInfo.floorPrice) {
              this.$message.warning('活动原价不能低于活动底价！')
              reject()
              return
            }
            ...
            if (this.activityId) {
              ...
              this.$http.put('/v1/OrganizationReduceWeb/updateOrganizationReduceWeb', params).then(res => {
                // 提示
                this.$message.success('修改砍价模板成功。')
                resolve()
              }).catch(() => {
                reject()
              })
              ...
              params.status = 0
              params.maxReduceCount = tempTemplateForm.prizeInfo.totalNumber // 砍价次数
              params.maxReduceUserCount = tempTemplateForm.prizeInfo.num // 砍价名额数
              params.originalPrice = tempTemplateForm.prizeInfo.originalPrice // 原价
              params.maxReducePrice = tempTemplateForm.prizeInfo.originalPrice - tempTemplateForm.prizeInfo.floorPrice // 可砍价
              ...
              this.$http.post('/v1/OrganizationReduceWeb/addOrganizationReduceWeb', params).then(res => {
                // 提示
